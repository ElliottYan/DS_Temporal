...
class ContextGate(nn.Module):
    """
    Context gate is a decoders module that takes as input the previous word
    embedding, the current decoders state and the attention state, and
    produces a gate.
    The gate can be used to select the input from the target side context
    (decoders state), from the source context (attention state) or both.
    """
